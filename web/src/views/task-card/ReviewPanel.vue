...
const props = defineProps({
  ...
  commercialClassification: { type: String, default: '' }, cardType: { type: String, default: '' },
});
...
const classificationBlocked = computed(() => props.classificationPending || (props.cardType === '11' && !props.commercialClassification));
async function load() {
  if (!props.cardId) return;
  [reviews.value, changes.value] = await Promise.all([reviewApi.list(props.cardId), taskCardApi.listChangeRecords(props.cardId)]);
}
function requireClassification(result) {
  emit('classification-required', result);
  ElMessage.warning('商务分类候选尚未确认');
}
async function submit() {
  failedChecks.value = [];
  if (classificationBlocked.value) { requireClassification(); return; }
  try {
    if (!props.commercialClassification) {
      const derivation = await classificationApi.derive(props.cardId);
      if (!derivation?.classification || derivation?.status === 'requires_confirmation' || derivation?.requiresManualConfirmation) {
        requireClassification(derivation); return;
      }
    }
    const card = await reviewApi.submit(props.cardId, { changeReason: changeReason.value });
    emit('status-change', card);
  } catch (error) {
    failedChecks.value = Array.isArray(error.data?.failedChecks) ? error.data.failedChecks : [];
    const classificationFailure = failedChecks.value.find((item) => item.check === 'h'
      || item.rejection === 'COMMERCIAL_CLASSIFICATION_CONFIRMATION_REQUIRED');
    if (classificationFailure) emit('classification-required');
...
async function releaseToJob() {
  ...
  try {
    ...
    ElMessage.success(result.message || '发布成功'); emit('released', result);
  } catch (error) { ElMessage.error(error.message || '发布至工包失败'); }
  finally { releasing.value = false; }
}
onMounted(load);
defineExpose({ reviewComment, changeReason, reviews, changes, failedChecks, releasing, canRelease, classificationBlocked, submit, decide, releaseToJob, load });
</script>

<template>
  <div data-testid="review-panel">
    <el-alert v-if="classificationBlocked" title="商务分类候选尚未确认，不能提交审核" type="warning" :closable="false" data-testid="classification-pending" />
    <div v-if="status === 'New'" class="review-actions"><el-input v-model="changeReason" placeholder="提交/变更原因" data-testid="review-change-reason" />
      <el-button type="primary" :disabled="classificationBlocked" data-testid="submit-review" @click="submit">提交审核</el-button></div>
    ...
    <div v-if="canRelease" class="review-actions"><el-button type="primary" :loading="releasing" data-testid="release-to-job" @click="releaseToJob">发布至工包并进入 JOB</el-button></div>
    <el-alert v-if="failedChecks.length" type="error" :closable="false" data-testid="review-failures">
      <template #title>提交审核未通过</template><ul><li v-for="(item,index) in failedChecks" :key="item.check || index">({{ item.check || String.fromCharCode(97 + index) }}) {{ item.label || item.code || '校验项' }}：{{ item.message || item.reason || item }}</li></ul>
    </el-alert>
