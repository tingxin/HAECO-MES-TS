...
const classificationOpen = ref(false); const classificationRequiresConfirmation = ref(false); const duplicateMessage = ref('');
...
const classificationPending = computed(() => classificationRequiresConfirmation.value
  || (card.cardType === '11' && !card.commercialClassification));
...
async function loadCard() {
  ...
  classificationRequiresConfirmation.value = card.cardType === '11' && !card.commercialClassification;
...
async function checkRevision() {
  ...
  try {
    ...
    duplicateMessage.value = result.duplicate
      ? (result.message || `工卡编号 ${card.taskNo} 的版本 ${card.revision} 已存在`)
      : '';
...
async function save() {
  ...
  try {
    ...
    if (mode.value === 'revise') {
      ...
      saved = await taskCardApi.update(result[0].id, { ...card, reason });
    } else if (!card.id) saved = await taskCardApi.create(card);
    else {
      if (!changeReason.value.trim()) { ElMessage.warning('变更原因必填'); return; }
      saved = await taskCardApi.update(card.id, { ...card, reason: changeReason.value.trim() });
    }
...
function onClassificationDerived(result) { classificationRequiresConfirmation.value = Boolean(result.requiresManualConfirmation || (result.candidates?.length > 1 && !result.classification)); }
function onClassificationConfirmed(result) { card.commercialClassification = result.classification; classificationRequiresConfirmation.value = false; }
...
defineExpose({ card, mode, readonly, activeTab, changeReason, duplicateMessage, classificationPending, save, checkRevision, applyTpc });
</script>
  ...
  <section class="task-card-editor" data-testid="task-card-editor-view" v-loading="loading">
    <el-card shadow="never">
      ...
      <el-tabs v-model="activeTab" data-testid="editor-tabs">
        ...
        <el-tab-pane label="工序" name="steps">
          <el-alert v-if="!card.id" title="请先保存工卡后维护工序" type="info" :closable="false" />
          <ProcessStepList v-else :card-id="card.id" :rows="steps" :readonly="authoringReadonly" @refresh="refreshCard" />
        </el-tab-pane>
        <el-tab-pane label="关联工卡" name="relations"><el-alert v-if="!card.id" title="请先保存工卡后维护关联" type="info" :closable="false" />
          <CardRelationPanel v-else :card-id="card.id" :rows="card.relations || []" :readonly="authoringReadonly" />
          <template v-if="card.cardType === '05'"><el-divider content-position="left">Lot List 关联</el-divider><LotLinkPanel v-if="card.id" :card-id="card.id" :readonly="authoringReadonly" @update="refreshBom" />
            <el-divider content-position="left">BOM Base（IR 卡 + Lot List 两来源）</el-divider><BomBaseTable :rows="bomBases" /></template></el-tab-pane>
        <el-tab-pane label="审核记录" name="review"><el-alert v-if="pendingRevision" title="请先保存新版本，再提交审核" type="info" :closable="false" /><ReviewPanel v-else-if="card.id" :card-id="card.id" :status="card.status" :revision="card.revision" :classification-pending="classificationPending" :commercial-classification="card.commercialClassification || ''" @status-change="onStatusChange" @classification-required="classificationOpen = true" @released="onReleased" /><el-empty v-else description="保存后可查看审核记录" /></el-tab-pane>
      </el-tabs>
    </el-card>
    <ClassificationConfirmDialog v-model="classificationOpen" :card-id="card.id" @derived="onClassificationDerived" @confirmed="onClassificationConfirmed" />
  </section>
</template>
