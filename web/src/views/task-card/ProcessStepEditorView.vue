...
const readonly = computed(() => String(route.query.mode || '') === 'view' || card.status !== 'New');
const imageComponents = computed(() => step.components.filter((item) => item.type === 'image' && item.payload?.url));
function clone(value) { return value == null ? value : JSON.parse(JSON.stringify(value)); }
function assignStep(value = {}) { Object.assign(step, { processId: '', skill: '', refDocId: null, descriptionZh: '', descriptionEn: '', captureItems: [], components: [], signatureRequirements: [], safetyWarning: '', visualCue: null, repairTips: '', isCritical: false }, clone(value)); }
async function load() {
...
function writablePayload(reason) {
  return { skill: step.skill || null, refDocId: step.refDocId || null, descriptionZh: step.descriptionZh, descriptionEn: step.descriptionEn, safetyWarning: step.safetyWarning, visualCue: clone(step.visualCue), repairTips: step.repairTips, isCritical: Boolean(step.isCritical), captureItems: clone(step.captureItems), components: clone(step.components).map((item, index) => ({ ...item, sortOrder: index + 1 })), signatureRequirements: clone(step.signatureRequirements), ...(reason ? { reason } : {}) };
}
...
function insertComponent(type) { if (!readonly.value) step.components.push(newComponent(type, step.components.length + 1)); }
...
<template>
  <section class="step-editor" data-testid="process-step-editor-view" v-loading="loading">
    ...
    <el-card shadow="never"><template #header><strong>工序组件</strong></template><ComponentInserter :types="enums.componentType || []" :readonly="readonly" @insert="insertComponent" /><ProcessComponentEditor v-for="(item,index) in step.components" :key="item.id || `${item.type}-${index}`" :model-value="item" :readonly="readonly" @update:model-value="updateComponent(index,$event)" @remove="removeComponent(index)" /></el-card>
